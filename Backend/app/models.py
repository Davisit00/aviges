...
class EmpresasRIF(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    id_empresas_transportes = db.Column(db.Integer, db.ForeignKey("Empresas_transportes.id"), nullable=False)
    id_rif = db.Column(db.Integer, db.ForeignKey("RIF.id"), nullable=False)
    is_deleted = db.Column(db.Boolean, default=False, server_default="0", nullable=False)
    created_at = db.Column(db.DateTime, server_default=db.func.getdate(), nullable=False)

    __table_args__ = (
        # Index filtrado para permitir reusar el RIF si is_deleted=1
        db.Index('idx_empresas_rif_unique_active', 'id_rif', unique=True, mssql_where=db.text('is_deleted = 0')),
    )
...
class GranjasRIF(db.Model):
    ...
    __table_args__ = (
        # Index filtrado para permitir reusar el RIF si is_deleted=1
        db.Index('idx_granjas_rif_unique_active', 'id_rif', unique=True, mssql_where=db.text('is_deleted = 0')),
    )
